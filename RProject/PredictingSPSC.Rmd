...
#Make crs object for terra::project
...
#First sample from pools
PWSvect <- terra::vect(PWS.transform)
PWS.point.pool8 <- PWS.transform %>% dplyr::filter(ID == "Pool 8") %>% st_sample(PWS.transform, size = 30, type = "random")
plot(PWS.point.randompool)
...
PWS.point.100poolregularsf <- st_sf(geometry = PWS.point.randompool)
PWS.point.100poolregularjoin <- st_join(PWS.point.poolregularsf, PWS.transform)
...
#Convert sample points to sf
PWSpecom100_sf <- sf::st_as_sf(PWS_ep_point)
#Then to SpatialPointsDataFrame
