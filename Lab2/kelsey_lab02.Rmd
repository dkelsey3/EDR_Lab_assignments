...
#If there is something similar that binds the two datasets, hence the name column, you can use the column to bind the datasets. 
```
...
glimpse(bmps)
#Delimiter: Data fields are separated by characters to indicate a structure of columns and rows
```
...
# spatial
counties <- sf::read_sf("County_Boundaries.shp") %>% sf::st_make_valid()
dams <- sf::read_sf("Dam_or_Other_Blockage_Removed_2012_2017.shp") %>% sf::st_make_valid()
streams <- sf::read_sf("Streams_Opened_by_Dam_Removal_2012_2017.shp") %>% sf::st_make_valid()
