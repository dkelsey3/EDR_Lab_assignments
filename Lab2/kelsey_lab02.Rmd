...
bmps %>% dplyr::filter(!is.na(Cost)) %>% group_by(StateAbbreviation) %>% 
...
#Both Cost and Total Amount Credited are heavily skewed to the left. There isn't a nice bell curve. Therefore, I believe both axes need transformed. 
...
#bmps %>% mutate(covercrop = str_count(BMP, "Cover Crop")) %>% summarize(max = max(covercrop)) 
#Checked to make sure that I could filter correctly.

bmps %>% 
  mutate(
    covercrop = str_count(BMP, "Cover Crop"),
  )%>% dplyr::filter(covercrop == 1) %>% 
...
dams.filt <- dams %>% dplyr::filter(., YEAR != "0" & YEAR != "NA")
dams.filt

dams.filt %>% ggplot(., aes(x = YEAR, y = STATE)) +
  geom_point() +
  labs(title= "Dams built",x= "Year", y = "State")
...
bmps
counties 
#Want to link these two together.
bmps$GeoNameCond <- str_sub(bmps$GeographyName, 1,2)
bmps

bmpfilt.1 <- bmps %>%  as_tibble() %>% group_by(GeoNameCond, BMP) %>% summarise(totalcost = sum(Cost, na.rm=TRUE), .groups = "drop")

cobmp <- left_join(counties, bmpfilt.1, by = c("STATEFP10"= "GeoNameCond"))
cobmp

cobmp %>% 
  mutate(
  wetland = str_count(BMP, "Wetlands"),
  ) %>% dplyr::filter(wetland == 1) %>% ggplot(., aes(x = STATEFP10, y= totalcost, fill = STATEFP10)) +
  geom_bar(stat = "identity") +   
 scale_fill_brewer(palette = "Set1") +
  labs(x="State", y="Cost", title="Total Cost of wetland BMPS per State")
...
#sort by Length KM
#Pull first 5 columns

streams %>% dplyr::arrange(desc(LengthKM)) %>% slice(1:5)
```
...
#Next, group the counties
#Calculate the total length 
#arrange then slice

st.co <- sf::st_intersection(streams, counties)
str(st.co)

st.co %>% as_tibble() %>% dplyr::select(-geometry) %>% group_by(COUNTYFP10) %>% summarise(totallength = sum(LengthKM)) %>% dplyr::arrange(desc(totallength)) %>% slice(1:3)
```
...
#plot(dams)
#plot(streams)
dast <- sf::st_nearest_feature(dams, streams)  
#x: The spatial object of class sf containing the set of features (can be points, lines, or polygons).
#y: The spatial object of class sf containing the point(s) for which the nearest feature is to be found.
dast

neardast <- streams$GNIS_Name[dast]
neardast
#In case the names of the streams are needed
```
...
dams %>% as_tibble() %>% dplyr::select(-geometry) %>% group_by(STATE) %>% dplyr::count()
```
