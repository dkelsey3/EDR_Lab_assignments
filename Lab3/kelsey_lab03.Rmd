...
#B01001Ge17	SEX BY AGE (TWO OR MORE RACES): Female: People who are Two or More Races -- (Estimate)
#B01001Gm17	SEX BY AGE (TWO OR MORE RACES): Female: People who are Two or More Races -- (Margin of Error)
...
#Note, B01001e35 appears to be the total population while B01001Ge24 are specifically the population between 25-29 years. These are the variables I will use to normalize my data. I did not use the total two or more races as I am interested in how many females between 25-29 who are Two or More Races exist in the total population of females. 

kotvw$normalized_by_population <- kotvw$B01001Ge17 / kotvw$B01001e35
...
#What would be an appropriate data classification scheme here?
...
sf::st_crs(kotvw)
# then reproject to north american equidistant conic
kotwv.projected <- kotvw %>% sf::st_transform(., "ESRI:102010")
# plot it again to make sure nothing broke
tmap::tm_shape(kotwv.projected) + tm_polygons()

  #1. Row-standardize the W

# make the neighborhood
nb.ko <- spdep::poly2nb(kotwv.projected, queen = TRUE)
nb.ko[[1]]
...
F25.2r.lag <- lag.listw(lw.ko, kotwv.projected$B01001Ge17)
F25.2r.lag
...
## use zero.policy = T because some polygons don't have neighbors
moran.plot(kotwv.projected$B01001Ge17, lw.ko, zero.policy=TRUE, plot=TRUE)
...
lw.ko.idw <- nb2listwdist(nb.ko, kotwv.projected, type="idw", style="W",
alpha = 0, dmax = NULL, longlat = NULL, zero.policy=NULL)
#Takes a very long time. 
#Note: Alpha is usually set to 0. If we use 1 or 2 it can cause the error: no finite general weights. Alpha is a a parameter for controlling the distance modelling. 
...
F25.2r.lag.IDW <- lag.listw(lw.ko.idw, kotwv.projected$B01001Ge17)
F25.2r.lag.IDW
...
## use zero.policy = T because some polygons don't have neighbors
moran.plot(kotwv.projected$B01001Ge17, lw.ko.idw, zero.policy=TRUE, plot=TRUE)
...
Moran's I is measuring autocorrelation by using both dependency of variables and coefficients. Moran's I is the measurement of the likelihood of nearby polygons to be related. The way this is measured is by using Moran's coefficient. Moran's coefficient: I = n/∑i∑jwij*∑i∑jwij(zi-z̄)(zj-z̄)/∑i(zi-z̄)^2. Where i and j represent the locations of interest, wij specifies the dependency or spatial weight between the locations of interest, z̄ is the mean of the two variable of interest, zi and zj represents the value of the variables at the locations of interest, n is the number of features, and ∑i∑jwij is the sum of all the spatial weights. When I > 0, there is positive spatial autocorrelation. When I < 0, there is negative spatial autocorrelation. Finally, when I = 0, there is no spatial autocorrelation.  

2. Describe in your own words: what is a spatially-lagged variable?

A spatially lagged variable is the product of a spatial weight matrix, measuring the variable's behavior in the neighborhood of a given observation. The spatially lagged variable is the average of the variable within a neighborhood. 
...
If an observation falls in the H-L quadrant, this indicates high values above average surrounded by values below average within the Moran plot. These are often associated with outliers. This is useful to detect if the values fall outside of the neighborhood. 
...
#I think you have to create a matrix
#https://crd230.github.io/lab7.html

moran_kotwv <- data.frame(
  original_values = kotwv.projected$B01001Ge17,
  lag = F25.2r.lag
)


ggplot(moran_kotwv, aes(x = original_values, y = lag)) +
...
#This does not have the designated areas.  
