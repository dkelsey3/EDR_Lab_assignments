...
#Add up all the counties, the land and the water of all counties to get the total total.
#Then, divide the land by the total total


d.counties.land <- d.counties %>% mutate(stateLWtotal = sum(ALAND10) + sum(AWATER10)) %>% mutate(landper = (100* ALAND10/stateLWtotal))
d.counties.land
#Step by step process:
#1: I first created column for the total area of land and water for the state.
#2: The second column is the percentage of the land area per county of the total area for the state.
#Note, I chose to mutate them twice as I wanted to see the information in separate columns
...
d.counties.water <- d.counties %>% mutate(stateLWtotal = sum(ALAND10) + sum(AWATER10)) %>% mutate(wat = (AWATER10/stateLWtotal))
#Followed same steps as above to get the total, but instead values are water area /total area

d.counties.water %>% group_by(STATEFP10) %>% dplyr::filter(wat == max(wat))
```
...
d.counties %>%
  as_tibble() %>% dplyr::select(-geometry) %>% # this line converts the data because of wonky geometry
  group_by(STATEFP10) %>%  summarize(n = n(), .groups = "drop")

#Notes from reading:
#summarize(n = n()), counts the number of rows per state. 
...
d.stations %>% mutate(length = nchar(STATION_NA))  %>% arrange(length) %>% slice_min(., nchar(STATION_NA))
#slice_min() selects the rows with the smallest values of a variable. In this case, Abram Creek and Dragon Swamp both had the same amount of characters, hence why there are two answers. 
...
d.counties %>% 
  ...
  geom_point(aes(color = STATEFP10), size = 3) +
  labs(title = "Land and Water Relationship for Each County", color = "State FP") +
  xlab("Land") +
  ylab("Water") +
  theme_minimal()
...
d.stations %>% 
  ...
  labs(title = "Drainage Area for monitoring stations") +
  ylab("Monitoring Stations Count") +
  xlab("Drainage Area") +
  theme_minimal()
...
d.stations 
st.stations <- sf::st_intersection(d.stations, d.counties)
st.stations
...
```{r Still need to do step B, C, and D}
...
inclass <- function(myfun){
  if(is.numeric(myfun)) {
  sorted <- sort(myfun)
  return(list(sorted, mean = mean(myfun), median = median(myfun), max = max(myfun), min = min(myfun)))
  }
...
st.stations <- sf::st_intersection(d.stations, d.counties)
st.stations
#removed counties that did not intersect with a station.

st.stations %>%
  as_tibble() %>% dplyr::select(-geometry) %>% # this line converts the data because of wonky geometry
  group_by(STATEFP10) %>% count(STATEFP10)
...
d.counties %>% dplyr::filter(STATEFP10 == 36) %>% mutate(area = sf::st_area(geometry)) %>% summarise(mean = mean(area))

#2088695951 [m^2] is the average area in New York

#Notes from AI overview on google. This is for my personal reference so I know what each function means. sf::st_area(geometry) is a function used to calculate the area of a polygonal geometry object, returning the area value based on the coordinate reference system (CRS) used by the geometry data; essentially, it tells you how much space a polygon occupies within its defined boundaries

#4.3 Calculate which state has monitoring stations with the greatest average drainage area (Drainage_A)
st.stations <- sf::st_intersection(d.stations, d.counties)
st.stations %>%
  as_tibble() %>% dplyr::select(-geometry) %>% # this line converts the data because of wonky geometry
  group_by(STATEFP10) %>% summarise(mean = mean(Drainage_A)) %>% slice_max(., mean)

#42 is Pennsylvania - checked with plot. 

#Note to self, sf::st_area(geometry) was not used for 4.3 as we are not looking for an average geometric area, we are looking for an average numeric value 
...
```{r}
 ddel <- sf::st_intersection(d.stations, del.counties)
 deld <- sf::st_intersection(del.counties, d.stations)
...
When the code was ran, the outputs are providing the same information, but in a different order. Using st_intersection, the following information is joining differently. For example, in deld, for each county, the intersecting station is found whiles for ddel, for each station, we find the intersecting county. I believe that if you were using different types of data, my answer would be different. For example, if you were intersecting a line with a polygon, you might find the line crosses two polygons where if you were intersecting a polygon with a line, the polygon will not reflect the remainder of the line. 


2. What did you find challenging in this lab? What was new?

Thinking through the questions and trying to figure out the best way to approach the question was by far the most difficult. I tend to try a bunch of code until something works and then I got back and try to figure out why the code works. I have found that approaching the question this way is not going to be helpful by any means. Rather, as Dr. Bitterman has mentioned, you have to think through the problem THEN design the code. I am happy I am developing this skill, but I do think that will be challenging for a while in the class. The new things I learned from this lab was designing functions. I have used functions in the past, but actually thinking through how to create a function that follows the rules was completely new to me. In addition, I have never understood dplyr and would often avoid using it as dplyr frustrated me in the past. It is crazy how different I feel about dplyr to where I am much more comfortable using the functionality. 

3. What types of activities would you like to see in labs this semester?

One thing I think would be interesting to see is potentially how to utilize resources online when we are completing our labs. I tend to use StackOverflow, ChaptGPT, or Reddit in the R community to ask my questions, but I think it would be nice to see how others utilize sources and what is helpful. I would also be really interested in learning how to manipulate data in R. Finally, I am really enjoying the process of learning how to think through the question and visualizing the data before putting it into R so I think doing more of that will help with critical thinking and will be helpful for graduate school when I get to the home stretch. 